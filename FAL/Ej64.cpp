...
void resolver(tDatos& sol, vector<bool>& marcas, tMatriz& matriz,int& solMejor,int k) {
    ...
    for (int i = 0; i < marcas.size(); i++) {
        ...
        if (!marcas[i]) {
            ...
            if (k == sol.sol.size() - 1) {
                ...
                resolver(sol, marcas, matriz, solMejor, k + 1);
            marcas[i] = false;
            sol.sumaT -= matriz[k][i];
        }
    }
...
bool resuelveCaso() {
    ...
    resolver(sol, marcas, matriz, solMejor, 0);
    cout << solMejor << endl;
    // escribir sol
